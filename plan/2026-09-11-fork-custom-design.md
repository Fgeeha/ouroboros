...
The branch was split on top of `managed/ouroboros` @ `5a0bd4a2`:

- Light theme: dropped. Upstream shipped its own Light / Dark / System
  appearance (`e3095714`, `114d39d8`); the fork uses it as is.
- Russian UI: `fork/i18n`, proposed upstream as PR #940 (head `fork/custom`).
- Reasoning display: `fork/reasoning`, fork-only for now.
- `fork/all` is the owner's working build: `fork/i18n` + `fork/reasoning` +
  Makefile, agent definitions and this plan.

The sections below describe the original 2026-09-11 design.
