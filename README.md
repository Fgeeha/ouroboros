...
[![Version 7.2.2](https://img.shields.io/badge/version-7.2.2-green.svg)](VERSION)
...
[download-macos-arm64]: https://github.com/razzant/ouroboros/releases/download/v7.2.2/Ouroboros-7.2.2.dmg
[download-windows-x64]: https://github.com/razzant/ouroboros/releases/download/v7.2.2/Ouroboros-7.2.2-windows-x64.zip
[download-linux-deb-amd64]: https://github.com/razzant/ouroboros/releases/download/v7.2.2/ouroboros_7.2.2_amd64.deb
[download-linux-rpm-x86_64]: https://github.com/razzant/ouroboros/releases/download/v7.2.2/ouroboros-7.2.2-1.x86_64.rpm
[download-linux-rpm-red80-x86_64]: https://github.com/razzant/ouroboros/releases/download/v7.2.2/ouroboros-7.2.2-1.red80.x86_64.rpm
[download-linux-appimage-x86_64]: https://github.com/razzant/ouroboros/releases/download/v7.2.2/Ouroboros-7.2.2-linux-x86_64.AppImage
[download-linux-x86_64]: https://github.com/razzant/ouroboros/releases/download/v7.2.2/Ouroboros-7.2.2-linux-x86_64.tar.gz
...
|---------|------|-------------|
| 7.2.2 | 2026-09-19 | **fix: host messages retain System voice.** Command replies, runtime diagnostics and lifecycle notices carry explicit authorship and type through live delivery and history. Model narration and proactive replies keep their own voice; named transport operations still recognize typed terminal replies. A structural producer test and consumer regressions pin the boundary. Existing terminal-host-notice and question-card contracts are unchanged. |
| 7.2.1 | 2026-09-19 | **fix: an optional argument that asks for nothing is no longer a refusal, reflection keeps the evidence, and plan review stops speaking for the wrong plan (#1114).** A filled optional value that asks for nothing now takes the omitted path with a single receipt line in `knowledge_read`, `schedule_followup` and `delegate_wait`, and a value that asks for something unservable is refused once, typed, naming the field and the repair in `schedule_followup` and `delegate_wait`; `get_task_result` publishes an invalid source range as an argument error naming the complete length and the range received instead of ok, and the `escalate` quiz refusal is typed and says the quiz was not sent. The receipt line and the typed refusal live in the new `ouroboros/tools/arg_feedback.py`. `knowledge_write` with an empty `expected_revision` creates a new note while overwrite protection on an existing note is unchanged, and a blocked link in `send_links` stays a policy denial and now states that no links were sent. Post-task reflection sees the calls and outcomes as the trace retained them, with a pointer to the exact stored record, compares full content when detecting repeats, and preserves the recovered status-classification corrections. In plan review a historical critic is evidence labelled as the earlier plan's and can no longer overwrite `cycles_exhausted`; an author stop is no longer described as work that proceeded under advisory enforcement; self-check outcomes are read from one trace row, so reused provider call ids cannot attach another call's error to a successful call after context compaction. The escalate and quiz wait bound keeps the 7.2.0 semantics. Known and disclosed in `docs/architecture/06-agent-core.md`: the web plan-review group can still present an earlier critic's verdict as its own and reads a spent review cap only from the wave. |
