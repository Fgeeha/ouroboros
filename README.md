...
[![Version 7.2.1](https://img.shields.io/badge/version-7.2.1-green.svg)](VERSION)
...
[download-macos-arm64]: https://github.com/razzant/ouroboros/releases/download/v7.2.1/Ouroboros-7.2.1.dmg
[download-windows-x64]: https://github.com/razzant/ouroboros/releases/download/v7.2.1/Ouroboros-7.2.1-windows-x64.zip
[download-linux-deb-amd64]: https://github.com/razzant/ouroboros/releases/download/v7.2.1/ouroboros_7.2.1_amd64.deb
[download-linux-rpm-x86_64]: https://github.com/razzant/ouroboros/releases/download/v7.2.1/ouroboros-7.2.1-1.x86_64.rpm
[download-linux-rpm-red80-x86_64]: https://github.com/razzant/ouroboros/releases/download/v7.2.1/ouroboros-7.2.1-1.red80.x86_64.rpm
[download-linux-appimage-x86_64]: https://github.com/razzant/ouroboros/releases/download/v7.2.1/Ouroboros-7.2.1-linux-x86_64.AppImage
[download-linux-x86_64]: https://github.com/razzant/ouroboros/releases/download/v7.2.1/Ouroboros-7.2.1-linux-x86_64.tar.gz
...
|---------|------|-------------|
| 7.2.1 | 2026-09-19 | **fix: an optional argument that asks for nothing is no longer a refusal, reflection keeps the evidence, and plan review stops speaking for the wrong plan (#1114).** `knowledge_write`, `schedule_followup`, `get_task_result`, `delegate_wait` and `send_links` share one rule through the new `ouroboros/tools/arg_feedback.py`: a filled optional value that asks for nothing takes the omitted path with a single receipt line, and a value that asks for something unservable is refused once, typed, naming the field and the repair. `knowledge_write` with an empty `expected_revision` creates a new note while overwrite protection on an existing note is unchanged, and a blocked link in `send_links` stays a policy denial instead of degrading the execution axis. Post-task reflection sees the calls and outcomes as the trace retained them, with a pointer to the exact stored record, compares full content when detecting repeats, and preserves the recovered status-classification corrections. In plan review a historical critic is evidence labelled as the earlier plan's and can no longer overwrite `cycles_exhausted`; an author stop is no longer described as work that proceeded under advisory enforcement; self-check outcomes are read from one trace row, so reused provider call ids cannot attach another call's error to a successful call after context compaction. The escalate and quiz wait bound keeps the 7.2.0 semantics. Known and disclosed in `docs/architecture/06-agent-core.md`: the web plan-review group can still present an earlier critic's verdict as its own. |
