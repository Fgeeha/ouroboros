...
// The fade says "there is more": it appears only when the fold really hides text.
// One read after mount; the clamp itself is unconditional CSS, so this never moves layout.
function markFold(bubble) {
    const message = bubble.querySelector('.message');
    if (message && message.scrollHeight > message.clientHeight + 1) bubble.classList.add('is-folded');
}
...
export function decorateProjectRow(bubble, { role = 'system', projectId = '', projectName = '' } = {}) {
    ...
    if (role === 'assistant') {
        ...
        if (typeof requestAnimationFrame === 'function') requestAnimationFrame(() => markFold(bubble));
    } else {
