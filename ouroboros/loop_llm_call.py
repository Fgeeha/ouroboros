...
def classify_llm_exception(exc: Exception, safe_error: str = "") -> LlmErrorClassification:
    ...
    # Its own kind, not `bad_request`: the request was well formed and the route
    # answered — with another model, which the caller refused. A different
    # account can serve the same model right now, so this must stay eligible for
    # the cross-model chain while never cooling the model the owner asked for.
    if getattr(exc, "code", "") == "model_substituted":
        return LlmErrorClassification("model_substituted", False, 0, exc.code)
...
def _record_llm_call_error(
    ...
    if classification.kind == "model_substituted":
        # The refusal names the horizon this round would have run under; the
        # disclosure owner reads it from here whether or not a redo recovered.
        context = (getattr(error, "problem", {}) or {}).get("context") or {}
        ctx.accumulated_usage["_model_substitutions"] = [{
...
def provider_no_call_source(accumulated_usage: Dict[str, Any], deadline_exhausted: bool) -> Tuple[str, bool]:
    ...
    # The same route already refused this exact conversation before dispatch, or
    # answered it with another model. A salvage call is the SAME request on the
    # SAME route, so it can only buy the same refusal a second time.
    if str(accumulated_usage.get("_last_llm_error_kind") or "") == "model_substituted" or (
            str(accumulated_usage.get("_last_llm_provider_code") or "") == "invalid_continuation"):
        return "same_route_refusal_no_resend", False
...
def call_llm_with_retry(
    ...
    for attempt in range(transient_budget):
        ...
        try:
            ...
            # Generations another model answered and the transport discarded on
            # the way to this one: an accepted answer still discloses them.
            substituted = (usage.get("claudexor") or {}).get("substituted")
            if isinstance(substituted, list) and substituted:
                accumulated_usage["_model_substitutions"] = list(substituted)
            context_fit_event_fields = _context_fit_event_fields(accumulated_usage) if physical_context is not None else {}
