...
BAND_PATHS = {
    ...
    "ouroboros/delegate_supervision.py": "The existing event-only supervision owner keeps observation cadence separate from the HTTP read allowance, preserving current controls and exact-run custody through transient read timeouts.",
    ...
    "ouroboros/gateway/claudexor_accounts.py": "Account-family transport keeps independent daemon facets and login receipts together at one HTTP boundary.",
    ...
    "ouroboros/project_dialogue.py": "Project dialogue remains the shared live and replay owner, including the additive owner-wait quiz field.",
    ...
    "ouroboros/review_evidence_sections.py": "The acceptance evidence assembler keeps individually materialized trajectory completeness and non-dict evidence disclosure beside final packet budgeting; the P3 and landed #736 paths share the same provenance decisions instead of a second evidence formatter.",
    ...
    "tests/test_llm_claudexor.py": "Keeps the caller-owned turn-state and failed-profile dispatch regression tests together; target and P3 composition crosses 1000 lines without adding runtime complexity.",
