...
`--expose-vulnerable-runtime`, the adapter mounts only the current task's
`vul` binary directory read-only at `/workspace/.cybergym-runtime`; it never
mounts the fixed directory, verifier database, mask map, or Docker socket.
A dynamic full image store is not required.
...
advisory lane disabled.  Task review runs in `off` mode, while enforcement is
...
3. **Continued monitoring.** Monitoring runs continuously from launch; reaching
   ...
   10--30 minutes and stops dispatch before the USD 3,000 cap when spend,
...
unresolved upper bounds must remain below USD 3,000.  A finished attempt
