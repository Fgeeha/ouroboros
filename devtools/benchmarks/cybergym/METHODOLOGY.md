...
never copied into this repository.  When explicitly enabled, the adapter gives
the agent only the current task's vulnerable binary directory as a read-only
mount at `/workspace/.cybergym-runtime`; fixed binaries, verifier data, mask
maps, reference PoCs, Docker control, and database files remain unavailable.
A dynamic full image store is not part of this methodology or PR.
...
lane disabled.  Task review is `off`, enforcement is `advisory`, and the
shared review-cycle value is `2` (at most two paid cycles).

The current owner-authorized cohort applies `OUROBOROS_MAX_ROUNDS=600`; the
...
they settle; admission is bounded by the lanes actually running.  Every
finished row is fsynced, settled, and acknowledged in completion order before
its lane is refilled.  ``result_index.jsonl`` therefore records completion
order; the manifest's pinned catalog and the launcher's returned summary retain
...
```text
settled_usd + reserved_usd + unresolved_upper_bound_usd <= 3000
```
...
USD 3,000, stop and report actual spend, throughput, uncertainty, and the
