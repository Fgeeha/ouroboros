...
def parse_args(argv: Sequence[str] | None = None) -> argparse.Namespace:
    ...
    parser.add_argument(
        "--expose-vulnerable-runtime",
        action="store_true",
        help="mount only this task's vulnerable binary runtime read-only in the agent workspace",
    )
    parser.add_argument(
        "--cybergym-python",
        default="",
    ...
    parser.add_argument("--budget-usd", type=float, default=DEFAULT_CAMPAIGN_BUDGET_USD)
...
def _build_default_executor(
    ...
    *,
    ...
    config = ExecutorConfig(
        ...
        expose_vulnerable_runtime=bool(getattr(args, "expose_vulnerable_runtime", False)),
    ...
    try:
        ...
        if isolated_server is not None and server_close_allowed:
            try:
                ...
                if isinstance(state_export, Mapping) and state_export:
                    extra["state_export"] = dict(state_export)
        extra["server_cleanup"] = server_cleanup
...
def _prepare_applied_settings(
    ...
    overrides: dict[str, Any] = {
        ...
        # The measured task does not spend model calls on acceptance review.
        # ``derive_disabled_tools`` independently withholds the manual review
        # tool so this applies to both automatic and model-initiated paths.
...
def main(argv: Sequence[str] | None = None) -> int:
    ...
    try:
        ...
        if str(getattr(args, "state_dir", "") or "").strip():
            assert_outside_repo(pathlib.Path(args.state_dir), repo_dir)
    ...
    try:
        manifest = admit_benchmark_run(
            ...
            harness={
                ...
                "expose_vulnerable_runtime": bool(
                    getattr(args, "expose_vulnerable_runtime", False)
                ),
    ...
    with contextlib.closing(campaign_lock_handle), \
        ...
        try:
            ...
            contract["vulnerable_runtime"] = {
                "enabled": bool(getattr(args, "expose_vulnerable_runtime", False)),
                "container_path": "/workspace/.cybergym-runtime",
                "read_only": True,
            }
