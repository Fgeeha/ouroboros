...
def test_skill_review_leaves_are_non_catalog_owners_without_backedges():
...
def test_upstream_cycles_names_stay_importable_from_the_facade():
    """Upstream re-homed the accepted-rebuttal ledger and the wave-budget refusal
    into ``skill_review_cycles`` (its own module-size split) before this
    extraction; the facade keeps their historical underscore aliases."""
    import ouroboros.skill_review_cycles as skill_review_cycles

    for alias, owner_name in {
        "_accepted_rebuttals_path": "accepted_rebuttals_path",
        "_fail_items_from_history_entry": "fail_items_from_history_entry",
        "_install_skill_dispatch_stamp": "install_skill_dispatch_stamp",
        "_load_accepted_rebuttals": "load_accepted_rebuttals",
        "_persist_rebuttal_flips": "persist_rebuttal_flips",
        "_record_accepted_rebuttal": "record_accepted_rebuttal",
        "_review_wave_budget_block": "review_wave_budget_block",
    }.items():
        assert getattr(skill_review, alias) is getattr(skill_review_cycles, owner_name), alias
