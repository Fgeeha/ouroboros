...
def test_run_llm_loop_preserves_assistant_tool_call_metadata(tmp_path, monkeypatch):
    ...
    result, _usage, _trace = run_llm_loop(
        ...
        llm=FakeLLM(),
        drive_logs=tmp_path,
        emit_progress=lambda _text, *, incident=None, meta=None: None,
...
def test_run_llm_loop_narrates_reasoning_to_bubble_not_trace(tmp_path, monkeypatch):
    ...
    result, _usage, trace = run_llm_loop(
        ...
        llm=FakeLLM(),
        drive_logs=tmp_path,
        emit_progress=lambda text, *, incident=None, meta=None: emitted.append(text),
        incoming_messages=queue.Queue(),
        task_id="narrate",
        drive_root=tmp_path,
    )
...
def test_run_llm_loop_injects_subagent_handoff_before_final_text(tmp_path, monkeypatch):
    ...
    result, _usage, trace = run_llm_loop(
        ...
        llm=FakeLLM(),
        drive_logs=tmp_path,
        emit_progress=lambda text, *, incident=None, meta=None: progress.append(text),
        incoming_messages=queue.Queue(),
        task_id="parent1",
        drive_root=tmp_path,
    )
