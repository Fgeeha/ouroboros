...
SCENARIOS = {
    ...
    # E8 ("budget-drain fail_tasks") was retired DELIBERATELY (F6 disposition,
    # owner Q10=A/Q9=A): pause is the one live semantics for a budget-exhausted
    # queued task, so the scenario re-enters under a new id instead of quietly
    # mutating in place.
    "E13": ("budget-drain pause before dispatch (supersedes E8)", LANE_PAID),
        ...
        if isinstance(fmt, dict) and fmt.get("type") == "json_object":
            ...
            if self.mode == "spawn" and self.spawned < 1 and "schedule_subagent" in names:
                ...
                call = {"name": "schedule_subagent", "arguments": json.dumps({
                    # delegation-by-construction (upstream): selection is
                    # explicit — the roster row isolated_settings() pins.
...
def isolated_settings(*, stub: StubModelServer | None, paid: bool = False, **overrides) -> dict:
    ...
    for slot in ("OUROBOROS_MODEL", "OUROBOROS_MODEL_HEAVY", "OUROBOROS_MODEL_LIGHT",
        ...
        cfg[slot] = slug
    # Upstream delegation-by-construction: schedule_subagent requires a SAVED
    # Available-subagents roster and an explicit subagent_id; one api_model row
    # on the lane's own slug keeps the cascade scenario on the same model the
    # rest of the lane runs (the stub passes subagent_id="mock-scout").
    cfg["OUROBOROS_SUBAGENTS"] = json.dumps({
        "enabled": True,
        "items": [{
            "subagent_id": "mock-scout",
            "name": "Mock scout",
