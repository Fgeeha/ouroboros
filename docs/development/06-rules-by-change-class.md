...
- Host acceptance: root-only, structured eligibility
  ...
  the answer (`tests/test_acceptance_optional_control.py`). A text-only rewrite under
  ready or pending PASS feedback is accepted without an extra panel or capacity
  refusal; a changed subject, owner source or other verdict takes the ordinary path
